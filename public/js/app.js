...
function filtrarSalas() {
  ...
  let visiveis = 0;
  document.querySelectorAll('#salas-grid [data-sala-card]').forEach(card => {
    const sala = salasCache.find(s => s.id === card.dataset.salaCard);
    const mostrar = sala && (!materia || sala.materia_id === materia) && normalizarBusca(`${sala.nome} ${sala.descricao || ''} ${parseAssuntos(sala.assuntos).join(' ')} ${nomesMaterias[sala.materia_id] || sala.materia_id || ''}`).includes(busca);
    card.hidden = !mostrar;
    if (mostrar) visiveis++;
  });
...
$('room-search').addEventListener('input', filtrarSalas);
...
async function renderProgressoPreview() {
  ...
  try {
    ...
    const piores = Object.entries(porAssunto).filter(([, v]) => (v.total || 0) > 0 && v.taxa < 100).sort((a, b) => a[1].taxa - b[1].taxa);
    if (piores.length) {
      const [assunto, v] = piores[0];
      fraseEl.innerHTML = `Uma boa próxima revisão: <strong>${escapeHTML(assunto)}</strong>. Você acertou ${v.taxa}% das questões desse assunto; retome as explicações para avançar.`;
    } else {
...
function mostrarView(id) {
  document.querySelectorAll('.home-extra').forEach(el => { el.hidden = true; });
...
async function carregarSalas() {
  ...
  if (!grid) return;
  const controles = ['room-search', 'room-subject', 'rooms-retry'].map($);
  ...
  try {
    ...
    const rooms = (d.rooms || []).filter(s => ['ACTIVE', 'PUBLISHED', 'CLOSED'].includes(s.status));
    salasCache = rooms;
    const materia = $('room-subject').value;
    $('room-subject').replaceChildren(new Option('Todas as matérias', ''));
    [...new Set(rooms.map(s => s.materia_id).filter(Boolean))].sort().forEach(id => $('room-subject').add(new Option(nomesMaterias[id] || id, id)));
    $('room-subject').value = [...$('room-subject').options].some(o => o.value === materia) ? materia : '';
...
function mostrarAviso(texto) {
  ...
  if ($('execucao').style.display === 'none') {
    let aviso = $('home-aviso');
    if (!aviso) { aviso = document.createElement('div'); aviso.id = 'home-aviso'; aviso.className = 'state'; aviso.setAttribute('role', 'status'); $('conteudo').prepend(aviso); }
...
async function abrirHistorico() {
  ...
  try {
    ...
    const piores = Object.entries(st.porAssunto || {}).filter(([, v]) => (v.total || 0) > 0 && v.taxa < 100).sort((a, b) => a[1].taxa - b[1].taxa);
    const fraseOrient = piores.length
      ? `<p class="pp-frase" style="margin-top:14px">Uma boa próxima revisão: <strong>${escapeHTML(piores[0][0])}</strong> (${piores[0][1].taxa}% de acerto) — retome as explicações para avançar.</p>`
      : '';
    statsEl.innerHTML = `
...
function carregarRodape() {
  ...
  if (el) el.textContent = 'Conhecimento que move você.';
}
